...
def main() -> None:
    ...
    if args.dry_run:
        ...
        for ide in ides:
            ...
            print(f"  Will copy: skills/, scripts/, mcp/, references/, roles/, stacks/")
            if ide == "claude":
    ...
    if not os.path.exists(skills_json) and os.path.exists(skills_example):
        ...
        print(
            f"  Copied skills.example.json → skills.json (edit to customize vendor skills)"
        )
    ...
    agents_md_dst = os.path.join(project_dir, "AGENTS.md")
    if os.path.exists(agents_md_src) and os.path.realpath(
        agents_md_src
    ) != os.path.realpath(agents_md_dst):
        import shutil as _shutil

        _shutil.copy2(agents_md_src, agents_md_dst)
        print("  AGENTS.md: copied to project root")

    # Install TAUSIK CLI wrapper
    import shutil
    import stat

    for wrapper in ("tausik_wrapper.sh", "tausik_wrapper.cmd"):
        src = os.path.join(_bootstrap_dir, wrapper)
        if os.path.exists(src):
            ext = os.path.splitext(wrapper)[1]
            dst = os.path.join(
                tausik_dir, f"tausik{ext}" if ext == ".cmd" else "tausik"
            )
