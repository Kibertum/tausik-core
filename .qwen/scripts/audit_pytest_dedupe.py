...
def collect_duplicates(repo_root: Path) -> list[dict[str, object]]:
    ...
    groups.sort(key=lambda g: (-len(g["members"]), g["members"][0]["file"]))  # type: ignore[index]
    return groups
...
def render_markdown(groups: list[dict[str, object]]) -> str:
    ...
    if not groups:
        ...
        total = sum(len(g["members"]) for g in groups)  # type: ignore[arg-type]
        lines.append(
        ...
        for i, g in enumerate(groups, 1):
            members = g["members"]
            lines.append(f"## Group {i} (sig `{g['signature']}`, {len(members)} tests)")
            for m in members:
