...
metrics cost [--since ISO] [--until ISO]   # SUM токенов/cost и COUNT по task (slug NULL исключены)
                                # Опционально тарифы: .tausik/config.json → "llm_pricing_usd_per_million": { "<model_id>": <USD за 1M токенов>, … }
                                # Отрицательные цены отбрасываются; неизвестный model_id → тарифа нет (null для агрегаторов).
