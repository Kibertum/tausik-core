...
4. **Run `/review` in subagent.** Read `agents/skills/review/SKILL.md`, paste full contents into Agent prompt with task slug, goal, AC, stack.
...
5. **Run `/test` in subagent.** Read `agents/skills/test/SKILL.md`, paste full contents into Agent prompt. Stack = task stack.
...
10. **Update docs (auto).** `git diff --name-only HEAD~1`. Structural changes in `scripts/`, `agents/`, `bootstrap/`, core → suggest updating `references/`. Skip `CLAUDE.md`, `QWEN.md`, `.cursorrules`. No structural changes → skip silently.
