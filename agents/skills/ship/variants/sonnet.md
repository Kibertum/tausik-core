...
**Invoke:** Read `agents/skills/review/SKILL.md`, pass FULL contents inline to the Agent (subagents can't read files):
...
Run full `/test` in subagent. Read `agents/skills/test/SKILL.md`, paste FULL contents into Agent prompt with `Stack: {stack}`.
...
Execute `/commit` skill: read `agents/skills/commit/SKILL.md`, follow full algorithm (stage → gates → message → confirm → commit → verify). Reference task slug in body.
...
After commit, `git diff --name-only HEAD~1`. Structural changes in `scripts/`, `agents/`, `bootstrap/`, core → suggest updating `references/` (only directly affected files like `architecture.md`, `project-cli.md`). Skip `CLAUDE.md`, `QWEN.md`, `.cursorrules` (managed by bootstrap). No structural changes → skip silently.
...
- **Push gate blocks:** Use `TAUSIK_ALLOW_PUSH=1` — this skill is authorized to push after confirmation.
