...
- **Собственный экспорт задачи вычитается из ПОКРЫТИЯ квитанции**
  (verify-handle-dies-on-a-tasks-own-export-file). Задача вправе объявить в
  `--relevant-files` свой файл `tausik/tasks/<slug>.md` — для задачи, продуктом
  которой являются записи (разбор, вердикты, решения, планирование), это
  естественная область. Раньше такое объявление делало закрытие невозможным
  В ПРИНЦИПЕ: прогон `verify` сам пишет в задачу (объявленную область, номер
  прогона, квитанцию), экспорт от этого меняется, и хендл отвергался с точной,
  но бесполезной формулировкой «the files this receipt covers have changed».
  Повтор не сходился — каждый прогон двигал хеш дальше (наблюдалось
  `870e9d910c7c -> edd0acd46e7b -> e8d9e08ecc20`). **Если ты попал в цикл
  verify/done — это был он, и кэш тут ни при чём.** Теперь ровно ЭТОТ ОДИН файл
  вычитается из покрытия (конвенция #409: проверка обязана вычесть бухгалтерию
  самого фреймворка), на обеих сторонах — и при записи прогона, и при
  предъявлении хендла.
  - Вычитается ТОЛЬКО свой экспорт. Чужой (`tausik/tasks/<другой-slug>.md`,
    `tausik/decisions/*.md`) остаётся законным предметом покрытия: текущий
    прогон его не трогает, и его хеш означает ровно то, что написано.
  - Граница названа вслух: ручная правка своего же экспорта между `verify` и
    закрытием больше НЕ замечается — тот же честный предел, что у исключения
    проекции выше.
  - Правка файла-ИСХОДНИКА между `verify` и закрытием отвергается как раньше;
    отказ теперь прямо говорит, что экспорт задачи не входит в покрытие и не он
    сдвинулся.
  - Область, состоящая ТОЛЬКО из своего экспорта, покрывает после вычитания
    ноль файлов и потому НЕ сертифицирует: отказ называет причину и оба выхода
    — объявить настоящие файлы или закрыться через `--no-file-changes`.
...
**УТВЕРЖДЕНИЕ «ДИРЕКТИВА ДОЕЗЖАЕТ ОДИН РАЗ» ОПРОВЕРГНУТО ЗАМЕРОМ.** Проверялось
вызовом, а не чтением:

| вопрос | ответ | чем получен |
|---|---|---|
| Несёт ли директиву SessionStart-хук? | **Нет.** 7263 байта контекста (~1815 токенов), маркера «Output economy» в них нет | прогон `session_start.py` |
| Где она тогда? | В ФАЙЛЕ ПРАВИЛ, который харнесс подаёт как инструкции проекта **в каждом запросе** | содержимое CLAUDE.md наблюдается в системном промпте спустя часы работы |
| Переживает ли компакцию? | **Да**, по построению: правила лежат в ПРЕФИКСЕ запроса, а компакция сокращает историю разговора | там же |
| Сколько стоит впрыск на ход? | ≤888 символов ≈ 222 токена при медианном контексте 276 702 — **0.08%** (до контракта ответа в 1.9 было 700 / 0.06%) | `CAVEMAN_DIRECTIVE_MAX_CHARS`, стережётся тестом |
| Окупается ли? | Потолок экономии ~2% против цены 0.08% — примерно **двадцатипятикратно** | замер задачи о режиме экономии |

Разово происходит НЕ доставка, а **запись файла**: всякий генератор правил
preserve-if-exists, и если файл уже был, директива в него не попадёт. Именно об
этом предупреждает `warn_output_mode_not_applied`, и с 1.9 паритет этого
предупреждения по всем генераторам закреплён тестом
`tests/test_rules_generator_warning_parity.py`: хост, чей генератор промолчит,
оставил бы пользователя в уверенности, что режим применён.

## Окупает ли себя сгенерированная половина CLAUDE.md (замер #249)

arXiv 2602.11988 (ETH Zurich, 438 задач, 4 агента): контекстные файлы,
СГЕНЕРИРОВАННЫЕ моделью, стоят +20–23 % и отнимают 0.5–2 % успеха;
написанные человеком дают +4 %. Наш файл правил гибридный: рукописные правила
плюс секция DYNAMIC (`## Current State`, хвост памяти, блок общих знаний),
которую `update-claudemd` перегенерирует каждую смену, а харнесс подаёт
КАЖДЫМ запросом. Прибор: `scripts/context_block_audit.py`. Пороги объявлены
до замера (журнал задачи): хвост памяти остаётся при ≥ 25 % смен с цитатой,
источником которой мог быть только хвост; блок состояния остаётся при < 75 %
смен, где агент перезапрашивает его в первые 8 вызовов.

**Цена за запрос** (CLAUDE.md на момент замера): рукописное 4067 Б ≈ 1017
токенов — 0.37 % медианного контекста 276 702; `Current State` 246 Б ≈ 62 —
0.02 %; хвост памяти 2750 Б ≈ 688 — 0.25 %; общие знания 1518 Б ≈ 380 —
0.14 %. Сгенерированная половина — 4514 Б, 0.41 % контекста за ход, не +20 %:
масштаб приговора статьи к нам не переносится, но вопрос «читает ли её
агент» он не снимает.

**Использование**, 94 транскрипта этого проекта с этой машины (42 смены
верхнего уровня и 52 транскрипта субагентов, у которых тот же файл правил):

| прокси | что считается | результат | порог | вердикт |
|---|---|---|---|---|
| A, блок состояния | `status` / `session current` / `session open` в первые 8 вызовов — перезапрос того, что блок уже несёт | 41 из 94 — **43.6 %** | < 75 % | остаётся |
| B, хвост памяти | агент цитирует id памяти/решения, которого до этого не было ни в одном результате инструмента и ни в одном сообщении человека транскрипта, и который хвост нёс хоть раз за историю CLAUDE.md (338 id за 198 коммитов) | 35 из 94 — **37.2 %**, 58 разных id | ≥ 25 % | остаётся |

Что зачтено НЕ было: 68 процитированных id, которых хвост не нёс никогда
(источник — блок релевантности при `task start`, вывод инструментов,
общая память или выдумка — прибор не различает и не засчитывает); номера
смен, прогонов verify, квитанций и попыток (`смена #232`, `Передача #226`) —
первая версия прибора засчитала их хвосту, проверка выборкой это поймала, и
число ниже 300 без слова «память/решение/норма» перед ним теперь не
считается id. Блок общих знаний (0.14 %) не имеет id и его чтение
НЕ ИЗМЕРЕНО. НЕ измерено главное — успех задач, метрика статьи: для него
нужен парный A/B на одном наборе задач с блоком и без, это отдельные прогоны
и решение владельца. По объявленным правилам обе измеренные части остаются;
режется ничего не будет по привычке — и не оставляется по привычке: повтор
тем же прибором `python scripts/context_block_audit.py <транскрипты>`.

## Соблюдение контракта ответа: базовая линия (замер #249)

Правило без замера послушания — та же проверка, которой не было. Прибор:
`scripts/response_contract_audit.py` — читает ответы ПОЛЬЗОВАТЕЛЮ (последний
текстовый блок агента перед следующим сообщением человека; ответ на
уведомление харнесса — не ответ) из транскриптов Claude Code этого проекта,
из роллаутов Codex с cwd этого проекта и из произвольного JSONL `{"text"}`.
Считает четыре маркера — ровно те четыре удаления, которые контракт велит
сделать перед отправкой, — по прозе после вырезания защищённого (код,
цитаты вывода, пути, строки AC/решений/root cause, теги харнесса):

| маркер | что ловит | RU/EN |
|---|---|---|
| `intent_opener` | первая строка объявляет намерение («I'll start with…», «Сейчас посмотрю…», «Начинаю с…») | да |
| `closing_recap` | абзац в последней трети — итог или любезность («In summary», «Итого:», «Let me know») | да |
| `side_branch` | абзац открывает боковую ветку («By the way», «Попутно:», «Заодно») | да |
| `empty_hedge` | слово неуверенности без названной неопределённости («probably», «скорее всего», «вроде») | да |

Рубрика лексическая и говорит об этом прямо: пустую оговорку от названной она
не отличает (тест `test_a_hedge_that_names_its_uncertainty_is_still_counted`
это закрепляет), «Попутно: …» с содержательной находкой считается веткой.

**ПОРОГ ОБЪЯВЛЕН ДО ЗАМЕРА** (журнал задачи): доля ответов хотя бы с одним
маркером < 10 % → рычаг послушания не строится, задача снимается.

**ЗАМЕР, СМЕНА #249**, на этой машине, ничего из корпуса в репозиторий не
попало: 451 ответ пользователю в 151 файле (Claude 317, Codex 134; медиана
2175 символов). `intent_opener` 1.1 %, `closing_recap` 0.2 %, `side_branch`
2.4 %, `empty_hedge` 3.3 %; **с хотя бы одним маркером — 7.1 %**. Ниже
порога: рычаг НЕ строится. Что этот замер НЕ утверждает: он мерит четыре
удаления, а не форму «done → verified by → left → your call» — соблюдение
формы остаётся незамеренным, и «контракт соблюдается» здесь не зачитывается.
Проверка чувствительности до финальной цифры: выборка первых и последних
двух слов всех ответов — самые частые открытия состояние-первые
(«Задача закрыта», «Состояние:», «Дерево чистое»), а не объявления;
формы «Начинаю с», «Открываю», «Беру» добавлены в рубрику по этой выборке.
Повторный замер тем же прибором:
`python scripts/response_contract_audit.py <транскрипты> --project <корень> --threshold 7.1`
— выход 1 означает «не лучше базовой линии».

## Крышка на вывод команды неизвестного размера

**ИЗМЕРЕНО НА СВОЁМ НАБОРЕ, СМЕНА #229**, прибором этого же релиза: рост контекста
отнесён к инструменту, чей результат его вызвал (дельта до следующего вызова
внутри одной смены и одного транскрипта).

| инструмент | вызовов | медиана прироста | p90 | прирост на парах | доля |
|---|---:|---:|---:|---:|---:|
...
**ЭТУ ТАБЛИЦУ НАДО ЧИТАТЬ ТОЧНО.** Прирост от вызова к вызову содержит ДВЕ вещи:
результат инструмента И собственный вывод модели на том же ходу (рассуждение,
текст, блок вызова). Доля в последнем столбце — это ОБЩАЯ дельта на парах с
данным инструментом, а не цена самого инструмента.

Разделено на тех же 14 048 парах, прирост 18 009 761:

| что именно | токенов | доля прироста |
|---|---:|---:|
| собственный ВЫВОД модели, все инструменты | 12 895 606 | **71.6%** |
| результаты инструментов и обрамление | 5 114 155 | 28.4% |
| из них результаты **команд Bash** | 4 740 033 | **26.3%** |

То есть главный рычаг экономии — ФОРМА ОТВЕТА САМОЙ МОДЕЛИ, а ограничение вывода
команд стоит 26.3%, а не две трети. Правило крышки от этого не отменяется и порог
не двигается: p99 считался по дельте, и дельта не пересматривалась.

Рост при этом **широкий, а не хвостовой**: верхний 1% вызовов Bash даёт лишь 6.8%
его прироста. Катастрофического единичного вывода в корпусе нет, потому что
харнесс Claude Code режет вывод инструмента сам; **на хосте без такой резки он бы
был**.

**ПРАВИЛО.** У команды, размер вывода которой неизвестен ЗАРАНЕЕ, вывод
ограничивается. Строчного лимита недостаточно: одна длинная строка
(минифицированный файл, JSON в одну строку, лог без переносов) переполняет
контекст, а счётчик строк показывает единицу.

Безлимитный вызов запрещён для:

| вызов | чем ограничивать |
|---|---|
| `cat <файл>` без диапазона | `Read` с `offset`/`limit`, либо `sed -n 'A,Bp'` |
| широкий `rg` / `grep -r` | `Grep` с `glob`/`path`, либо `\| head -c 24000` |
| `find` по дереву | сузить `-maxdepth`/`-name`, либо `\| head -50` |
| `ls -R` | `Glob` с шаблоном |
| `git diff` целиком | `--stat`, затем диапазон по файлам |
| `git log` без `-n` | `-n <N>` и `--format` |

**ПРОВЕРЯЕТСЯ СРАБАТЫВАНИЕМ, А НЕ ЗАПИСЬЮ.** PostToolUse-хук
`tool_output_truncation_nudge` считает и строки (порог 250), и байты (порог
24 000, выставлен по измеренному p99 = 6 386 токенов). Он СОВЕТУЕТ, а не
блокирует: результат инструмента не меняется, вызов не отменяется. Пороги
настраиваются ключами `tool_output_truncation_threshold` и
`tool_output_truncation_bytes` в `.tausik/config.json`. Мутационные проверки —
`tests/test_output_byte_cap.py`, включая случай «одна строка, 50 КБ», который
строчный порог не видит.

**ЧУЖУЮ ЦИФРУ НЕ ПОВТОРЯЕМ.** Внешний источник (github.com/Austin1serb/agents-md)
называет байтовую крышку своим главным выигрышем и заявляет около −50% токенов.
Эта цифра **чужая, у нас не проверена и своей не объявляется** — та же оговорка
уже сделана про «~65%» caveman. Наше измеренное утверждение здесь ровно одно:
результаты команд Bash дают 26.3% прироста контекста, и у части этих вызовов
размер вывода неизвестен заранее.
