...
**И — вопреки прежней формулировке — обёртка `bash -c` / `sh -c`.** `bash -c
'echo x > scripts/foo.py'` не даёт НИ ОДНОЙ цели: перенаправление живёт внутри
одного закавыченного аргумента, а разбор внутрь payload'а интерпретатора не
идёт. Здесь раньше стояло, что гейт поднимает цену обхода до «нужно намеренно
обфусцировать»; это было преувеличением — `bash -c` пишут каждый день, и
обфускацией он не является. Формулировка исправлена, дыра названа и заведена
отдельной задачей (`bash-c-wrapper-bypasses-qg0-write-gate`), а не оставлена
под успокаивающим описанием. Честная нынешняя граница: гейт поднимает цену с
«heredoc в одну строку» до «обернуть команду в интерпретатор».
