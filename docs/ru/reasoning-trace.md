...
receipts). Сегодня трейс читается через `tausik task show`; более полный
таймлайн `tausik task replay`, сшивающий все источники, — в планах (задача
`v16r-task-replay`).
...
- **[CLI-команды](cli.md)** — `task reason-step`
- **[MCP-инструменты](mcp.md)** — `tausik_reason_step`
