...
После bootstrap идут **12 core skills** из `agents/skills/` (плюс `/brain` *условно* — когда у проекта настроен Notion, см. [Shared Brain](shared-brain.md)). Дополнительные **official / vendor skills** (25+) ставятся по запросу: per-skill через `tausik skill install <name>`, либо вся пачка через `python .tausik-lib/bootstrap/bootstrap.py --include-official` (alias `--include-vendor`). **Карта репо-скиллов:** [Экосистема скиллов (one-pager)](skill-ecosystem.md).

> **Изменение default в v1.4.x.** До v1.4.x bootstrap автоматически разворачивал все 38 source+registry скиллов (~1,520 токенов в system-reminder). С v1.4.x default — 12 + brain conditional (~480 токенов), экономия ~1,040 токенов на ход. Перезапусти bootstrap с `--include-official`, если нужен старый полный набор.
