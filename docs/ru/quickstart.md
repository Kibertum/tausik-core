...
> **`task_done_v2` (предпочитаем с 1.3.7).** Когда MCP-сервер агента публикует `tausik_task_done_v2`, он возвращает структурированный JSON-отчёт (`stage`, `gate_results`, `blocking_failures`), по которому агент чинит ошибки без парсинга прозы. Старые сборки серверов используют legacy `tausik_task_done` (одна агрегированная строка ошибки). Оба варианта уважают Verify-First Contract и читают один и тот же кеш.
