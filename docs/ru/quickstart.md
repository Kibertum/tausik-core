...
Есть два способа — через терминал или через Claude Code. Рекомендуем второй.

### Вариант А: через Claude Code (проще)

Откройте проект в VSCode, откройте Claude Code и напишите:

```
подключи фреймворк TAUSIK из https://github.com/Kibertum/tausik-core
как git submodule в .tausik-lib, запусти bootstrap
```

Claude сам выполнит все команды: добавит submodule, запустит bootstrap,
настроит `.gitignore`. Вам нужно только подтверждать его действия.
...
# Замените "my-project" на короткое имя вашего проекта (латиницей, без пробелов, например "web-app")
...
**После bootstrap перезапустите окно IDE** (Claude Code, Cursor, Windsurf). Bootstrap генерирует `.mcp.json` с конфигурацией MCP-сервера, но IDE считывает его только при запуске. Без перезапуска агент будет работать через CLI вместо MCP-инструментов.
...
2. **`--smart` автоматически определяет окружение:**
