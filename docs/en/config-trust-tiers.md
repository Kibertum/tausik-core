...
untrusted and both may only tighten; what separates them is reach.
`.tausik/config.json` is generated by bootstrap and carries machine-local data
(`_meta.lib_commit`, `installed_skills`, absolute paths), so a consumer project
commits it and TAUSIK's own repository ignores it wholesale — `.gitignore` has a
bare `.tausik/` rule. For four releases that meant this repository's own
tightenings existed in exactly one working copy. Measured on a fresh worktree of
HEAD against a user tier with `task_done.auto_verify: true`: `auto_verify`
resolved to **True** and `gates.bootstrap_drift.enabled` to **False**, with zero
rejections to show for it, because an absent project tier has nothing to reject.

`tausik/policy.json` — the non-dotted, branch-coupled projection that already
carries `gates.json` — is where a tightening that must survive cloning belongs.
It is composed UNDER `.tausik/config.json`: the local file wins on ordinary keys,
and on a **guarded** key the stricter of the two wins, so a gitignored file cannot
quietly undo a committed rule. The file is optional; a project without one
resolves exactly as it did before. See decision #287.
