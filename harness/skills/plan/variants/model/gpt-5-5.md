...
- **Aggressive parallel tool calls** — GPT-5.5 handles 8+ in one turn. Front-load all read-only ops (`memory_search`, `task_list`, `status`) into a single fan-out before any decision.
