...
- **QG-2 Implementation Gate.** `task done --ac-verified` требует evidence в notes + прохождения quality gates (pytest, ruff). Залогируй проверку AC через `task log` перед закрытием.
...
| Gates | `project_config.py` + `gate_runner.py` | Quality gates конфигурация + выполнение |
...
| QG-2 Implementation Gate | `task done` = evidence + --ac-verified + gates (no bypass) | Hard (CLI + MCP, --force удалён) |
...
| Rule 5 Verification Checklist | 28-item checklist, 4 тира (lightweight/standard/high/critical) | Warning |
...
.tausik/tausik task done <slug> --ac-verified # завершить (QG-2: evidence + gates)
...
## Стеки
20 значений: python, fastapi, django, flask, react, next, vue, nuxt, svelte, typescript, javascript, go, rust, java, kotlin, swift, flutter, laravel, php, blade.
Гайды в `agents/stacks/{stack}.md`.

<!-- DYNAMIC:START -->
## Current State
Session: #23 (active) | Branch: main | Version: 1.1.0
Tasks: 297/297 done, 0 active, 0 blocked
<!-- DYNAMIC:END -->
