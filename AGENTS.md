...
<!-- DYNAMIC:START -->
## Current State
Session: #137 (active) | Branch: release/1.8-batch-s126 | Version: 1.7.0
Tasks: 1064/1146 done, 0 active, 0 blocked
...
Decisions (5):
- #173 Слаги decisions/memory — транслитерация кириллицы в ASCII-латиницу, не Unicode и не только-id. Спека team-state-in-git.m
- #172 Состояние TAUSIK-проекта делится в команде через git-native файлы В ВЕТКЕ (по файлу на сущность, md+frontmatter), НЕ чер
- #171 Отличие «застрял» от «долгий» встраивается в САМ прогон (faulthandler_timeout=60 + faulthandler_exit_on_timeout=true в p
- #170 Языковой паритет детекторов evidence достигается ВЫРАВНИВАНИЕМ ПО НЕСТРОГОСТИ (русские варианты — такая же голая основа,
- #169 Гейт, читающий текст, который пишет сам проверяемый, обязан опираться на факт, проверяемый независимо (существование опр
Conventions (5):
- #306 Прокси-метрика обязана вычитать делопроизводство: то, что трогает КАЖДАЯ задача, не отличает большую
- #305 Проверка, сужающая область, обязана печатать ЗНАМЕНАТЕЛЬ рядом с вердиктом — иначе вердикт прочитают
- #302 Детектор, читающий прозу проверяемого, обязан говорить на языке, на котором эта проза пишется — и со
- #301 Перенося детектор на новый диалект/канал, переноси вместе с ним список ложных срабатываний, который 
- #299 Надзор бывает дырявым по КАНАЛУ, а не по правилу — и тесты этого не видят, потому что спрашивают тот
Dead ends (3):
- #290 Перенести is_tausik_project() выше skip-telemetry в task_gate.py, scope_write_gate.py, memory_pretoo
