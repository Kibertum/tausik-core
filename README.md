...
[![4095 tests](https://img.shields.io/badge/tests-4095-brightgreen.svg)](#proof-tausik-built-tausik)
...
- **4095 tests** — the enforcement core is the most-tested part.
...
- **123 MCP tools** (116 project + 7 brain) — full programmatic access to the project database.
...
Hooks — the real-time rails (no code without a task, bash firewall, push gate) — run in **Claude Code and Qwen Code**. Cursor, Windsurf and other MCP hosts get the same 123 tools and skills, with quality gates enforced at `task start` and `task done`.
...
v1.5 is a hardening release on the road to 2.0: signed receipts, fail-closed gates, external adversarial review for high-risk closures, closure-risk scoring, structured root cause. The enforcement core is **locked and covered by 4095 tests**, dogfooded daily. On uncommon paths you may still hit doc-vs-behavior drift — if you do, [file an issue](https://github.com/Kibertum/tausik-core/issues) and we'll converge it before 2.0.
